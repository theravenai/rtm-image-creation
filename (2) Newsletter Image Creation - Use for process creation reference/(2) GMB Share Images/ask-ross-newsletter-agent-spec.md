# Ask Ross Newsletter — Image Agent Spec

**For:** Raven AI agent (Claude Code) — monthly newsletter share graphic workflow
...
| If the newsletter is about... | Use this workflow |
|---|---|
| A named politician (Carney, Trump, etc.) or major global news event (war, election) | **Sonar** `build_newsletter_sonar.py` |
| You already have a specific photo | **Custom** `build_newsletter_custom.py` |
| Everything else — housing market, rates, mortgages, lifestyle, seasonal | **AI** `build_newsletter.py` |
| Generic topic with no strong news hook | **Pexels** `build_newsletter_pexels.py` |

**Default for most issues: AI.** Use Sonar only when a specific real person or major world event is the clear subject of the newsletter.

---

## Step 1 — Prepare the text fields (all workflows)
...
- Line 2 must be the longest line — the script auto-balances this (short / long / short shape)
- Line 3 minimum 3 words — script auto-enforces; line 3 can have more than 3 words

---

## Workflow A — Sonar (default for real people + current events)
...
python build_newsletter_sonar.py \
  --list-images \
  --sonar-query "Mark Carney Canada Prime Minister 2025" \
  --recency week
```

This prints each candidate with dimensions and aspect ratio. Look for entries marked **16:9 widescreen - good**.

**Query tips:**

| Topic | Example query |
|---|---|
| Political figure | `Mark Carney Canada Prime Minister 2025` |
| Policy event | `Bank of Canada rate cut 2025` |
| Trade/economic news | `US Canada tariffs 2025` |

Short and targeted beats long and descriptive. Always include the year.

### 2. Pick the best image

- **16:9 widescreen** (ratio ≥ 1.7) is ideal — marked in the list
- Clean editorial shot: subject at podium, speaking, or in action
- No baked-in text overlays, rally banners, or watermarks
- If the list has no good 16:9, use `--image-index N` to force a specific candidate
...
**`--use-cache` is required after `--list-images`** — ensures you get the exact same candidate list you just inspected, no new API call.

### Sonar flags

| Flag | Purpose |
|---|---|
| `--sonar-query` | Short, targeted search query |
| `--recency` | `day` / `week` / `month` / `year` — default: `month` |
| `--image-index N` | Force a specific candidate. Default: auto-pick best 16:9. |
| `--image-count N` | How many candidates to find (default 10) |
| `--list-images` | Print candidates and cache them — always run this first |
| `--use-cache` | Reuse the candidate list from `--list-images` |
| `--reuse-banner` | Skip search, reuse cached Banner PNG — text/layout tweaks only |
| `--banner-zoom 1.2` | Zoom into lower portion of banner (1.0 = none) |

---

## Workflow B — Custom (you have a specific photo)

Drop your image into `in/` then:

```bash
python build_newsletter_custom.py \
  --banner-image "in/your-photo.jpg" \
...
| Flag | Purpose |
|---|---|
| `--banner-image PATH` | Path to image (relative to script dir or absolute) |
| `--reuse-banner` | Skip image load, reuse cached Banner PNG |
| `--banner-zoom 1.2` | Zoom into lower portion of banner |

---

## Workflow C — AI (lifestyle, seasonal, abstract topics)

### 1. Generate the banner prompt (Art Director)

Use the prompt-builder template below. Read the newsletter, identify the dominant theme, emotional register, and scene mode, then output a single image prompt line.

---

#### Art Director Prompt Template
...
python build_newsletter.py \
  --banner-prompt "<output from Art Director step>" \
  --title1 "Issue Title Here" \
  --intro "Two to three sentence summary here."
```
...
| Flag | Purpose |
|---|---|
| `--banner-prompt` | Image prompt (from Art Director step above) |
| `--reuse-banner` | Skip API call, reuse cached Banner PNG — text/layout tweaks only |
| `--banner-zoom 1.25` | Zoom into lower portion of banner |

---

## Workflow D — Pexels (stock photo backup)

```bash
# Browse first
python build_newsletter_pexels.py \
  --list-photos \
  --pexels-query "Donald Trump press conference"

# Build with chosen photo
python build_newsletter_pexels.py \
  --pexels-query "Donald Trump press conference" \
  --photo-index 0 \
  --title1 "Your Title" \
...
## Text/layout tweaks (any workflow)

To adjust text without regenerating the banner image, add `--reuse-banner` to any build command:

```bash
python build_newsletter_sonar.py \
  --reuse-banner \
  --title1 "Adjusted Title" \
  --intro "Adjusted intro text."
```
...
| Image credits (Sonar/Pexels) | `{title1} Credits.txt` |

Colons in title1 become ` - ` for Windows compatibility.
...
```
GMB Image Builder/
├── build_newsletter_sonar.py     <- Sonar workflow (default)
├── build_newsletter_custom.py    <- Custom image workflow
├── build_newsletter.py           <- AI image workflow
├── build_newsletter_pexels.py    <- Pexels stock workflow
├── .env                          <- API keys
├── assets/
...
| Key | Used by |
|---|---|
| `PERPLEXITY_API_KEY` | Sonar workflow |
| `PEXELS_API_KEY` | Pexels workflow |
| `OPENROUTER_IMAGE_API_KEY` | AI workflow |
...
|---|---|---|---|---|
| BANNER region | 0 | 0 | 1200 × 638 | Hero photo |
...
| `title1` | Yes |
| `intro` | Yes |
| Search query / banner prompt / image path | Yes — depends on workflow |
| `title2` | No — locked |
